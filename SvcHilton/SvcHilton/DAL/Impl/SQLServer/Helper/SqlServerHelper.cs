...
namespace DAL.Impl.SQLServer.Helper
{
    ...
    /// </summary>
    public class SqlServerHelper : IDataBaseSQLHelper
    {
        ...
        public DataSet ExecuteQuery(string sentenceSql)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new DALException(String.Format("The sentence {0} can not be executed", sentenceSql), ex);
            }
            finally
            {
                if (adapter != null)
        ...
        public DataSet ExecuteProcedureToDataSet(string procedureName, IList<SqlParameter> parameters)
        {
            ...
            {
                Trace.WriteLine(ex.Message);
                throw new DALException(String.Format("The procedure {0} can not be executed", procedureName), ex);
            }
            finally
            {
        ...
        public DataSet ExecuteSqlToDataSet(string sentenceSql, IList<SqlParameter> parameters)
        {
            ...
            {
                Trace.WriteLine(ex.Message);
                throw new DALException(String.Format("The procedure {0} can not be executed", sentenceSql), ex);
            }
            finally
            {
                Common.CreateTrace.WriteLog(Common.CreateTrace.LogLevel.Debug, "----------------------------------------- FIN");
